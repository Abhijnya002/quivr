...
def gr_check(data, source_type, destination_type, tenant_id="", timeout=5.0, **context):
    ...
    if _prior:
        try:
            hop_label = gr_check(hop_label, "agent", "log", site_id='site:sha256:6f175afa80328c2137f8d369e2a507e78f4b8110d4f09dbd01b299cb090ecd6e')
    ...
    if _blk(data) or _blk(context):
        try:
            hop_label = gr_check(hop_label, "agent", "log", site_id='site:sha256:e62eba69195f097d9402ac4acdb863e4436a5777d50a0c7094b4ceb3d6794313')
        except Exception as _gr_exc:
            if type(_gr_exc).__name__ == "GRBlockedError": raise
            hop_label = hop_label
            __import__("logging").getLogger("lineaje.gr_client").warning("Lineaje guardrail unavailable at 'agent->log' — passing data through unchecked")
    ...
    if not url:
        try:
            data = gr_check(data, "agent", "user_interface", site_id='site:sha256:91636e7a270f2766638815b3c2ce84fb01a5278353cab5dfdc2bbb048b9e3274')
        except Exception as _gr_exc:
            if type(_gr_exc).__name__ == "GRBlockedError": raise
            data = data
    ...
    try:
        ...
        if isinstance(exc, _ue.HTTPError) and exc.code == 403:
            ...
            try:
                hop_label = gr_check(hop_label, "agent", "log", site_id='site:sha256:0759709676340d2c71f77e998208e8eccd8cf0252addae39f0667f72388aeca4')
            except Exception as _gr_exc:
                if type(_gr_exc).__name__ == "GRBlockedError": raise
                hop_label = hop_label
                __import__("logging").getLogger("lineaje.gr_client").warning("Lineaje guardrail unavailable at 'agent->log' — passing data through unchecked")
        ...
        try:
            hop_label = gr_check(hop_label, "agent", "log", site_id='site:sha256:f46031f99b97a0d698ec5661f296aed55412a51992645bd2348660f5aed010ab')
        except Exception as _gr_exc:
            if type(_gr_exc).__name__ == "GRBlockedError": raise
            hop_label = hop_label
            __import__("logging").getLogger("lineaje.gr_client").warning("Lineaje guardrail unavailable at 'agent->log' — passing data through unchecked")
    ...
    with open("response.jsonl", "w") as f:
        async for event in conversational_qa_chain.astream_events(
            ...
            if (
                ...
                try:
                    import asyncio as _gr_asyncio
                    dict_chunk = await _gr_asyncio.to_thread(gr_check, dict_chunk, "agent", "external", site_id='site:sha256:4662829debaa57193ea0db8e4f007b5183516bf0029f7e570aeb1c0f0e300c6f')
                except Exception as _gr_exc:
                    if type(_gr_exc).__name__ == "GRBlockedError": raise
                    dict_chunk = dict_chunk
                    __import__("logging").getLogger("lineaje.gr_client").warning("Lineaje guardrail unavailable at 'agent->external' — passing data through unchecked")
