...
class ProcessorBase(ABC, Generic[R]):
    ...
    async def process_file(self, file: QuivrFile) -> ProcessedDocument[R]:
        ...
        try:
            ...
            _lineaje_payload = await _gr_asyncio.to_thread(gr_check, _lineaje_payload, "agent", "log", site_id='site:sha256:6d1d600f39af5671b2ce31c5478639545fa240e5d7310fe7f0aeaae54a009672')
        ...
        try:
            import asyncio as _gr_asyncio
            docs = await _gr_asyncio.to_thread(gr_check, docs, "agent", "user_interface", site_id='site:sha256:6677b0eb18904941291375d81016148105b12d996562a343fde55443e3be087a')
        except Exception as _gr_exc:
            if type(_gr_exc).__name__ == "GRBlockedError": raise
            docs = docs
